...
from mappers import (domain_to_schema_message,
                     map_domain_to_message_out,
                     schema_to_domain_message)
from api.message_schemas import MessageIn, MessageOut


async def send_message(repo: IMessageRepository,
                       msg_in: MessageIn,
                       from_user_id: UUID):
    try:
        ...
        msg_out_receiver = domain_to_schema_message(
            saved_msg,
            current_user_id=saved_msg.from_user_id)
        msg_out_receiver.tags = msg_in.to_user_tags or []
        msg_out_receiver.type = 'received'

        msg_out_sender = domain_to_schema_message(
            saved_msg,
...
async def get_chat_between_users(
        ...
        with_user_id: UUID,
        last_id: UUID = None):
    messages = await repo.get_messages_by_chat(
...
async def get_messages_by_tag_use_case(
        repo: IMessageRepository,
        user_id: UUID,
        tag_id: UUID) -> List[MessageOut]:
    domain_messages = await repo.get_messages_by_tag(
        user_id=user_id,
        tag_id=tag_id)
