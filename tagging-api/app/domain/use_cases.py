...
async def get_message_tags(db:AsyncSession, user_id:UUID, text:str):

    user_tags = await get_tags(db, user_id)
    user_tag_names = [tag.name for tag in user_tags]

    if user_tag_names:
        from_user_response = classifier_service.tag_message(text=text, tags=user_tag_names)
        if from_user_response:
