...
from app.api.schemas import (UserCredentials,
                             Token, UserCreate,
                             UserSearch, UsersIdRequest)
from app.domain.use_cases import (get_user_by_id, register_user, login_user,
                                  search_users, search_users_list)
...
@router.get("/search/user-id{user_id}", status_code=status.HTTP_200_OK,
            response_model=UserSearch)
async def search_users_by_id_route(
    user_id: str,
    _: User = Depends(get_current_user),
    user_repo: UserRepository = Depends(get_user_repository)
):
...
@router.post("/search/users", status_code=status.HTTP_200_OK,
             response_model=List[UserSearch])
async def search_users_list_route(
